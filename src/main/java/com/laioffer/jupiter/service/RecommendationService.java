...
public class RecommendationService {
    ...
    // E.g., if a user favorites some videos about game "Just Chatting", then it will return some other videos about the same game.
    private List<Item> recommendByFavoriteHistory(Set<Item> favoritedItems, List<String> favoritedGameIds,
                                                  ItemType type) throws RecommendationException {
        // Count the favorite game IDs from the database for the given user.
        // E.g. if the favorites game ID list is ["1234", "2345", "2345", "3456"], the returned Map is {"1234": 1, "2345": 2, "3456": 1}
        ...
        // Search Twitch based on the favorite game IDs returned at the last step.
        ...
        for (Map.Entry<String, Long> favoriteGame : sortedFavoriteGameIdListByCount) {
            ...
            for (Item item : items) {
                ...
                // If the item is already favored by the user, we do not recommend it anymore
    ...
    public Map<String, List<Item>> recommendItemsByUser(String userId) throws RecommendationException {
        ...
        Set<Item> favoriteItems;
        Map<String, List<String>> favoriteGameIds;

        favoriteItems = favoriteDao.getFavoriteItems(userId);
        ...
        for (Map.Entry<String, List<String>> entry : favoriteGameIds.entrySet()) {
            if (entry.getValue().size() == 0) {
                ...
                recommendedItemMap.put(entry.getKey(),
                        recommendByFavoriteHistory(favoriteItems, entry.getValue(), ItemType.valueOf(entry.getKey())));
            }
        }
